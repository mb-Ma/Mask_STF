...
class Trainer(object):
    ...
    def val_epoch(self, epoch, val_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                loss = self.loss(output.cuda(), label) / (data.shape[0]*data.shape[1]*data.shape[2])
    ...
    def train_epoch(self, epoch):
        self.model.train()
        total_loss = 0
        for batch_idx, (data, target, mask) in enumerate(self.train_loader):
            data = data[..., :self.args.input_dim]
            ...
            # do mask
            # import pdb; pdb.set_trace()
            mask_row, mask_col = torch.where(mask==0) # mask[:, 1, :, 1]
            for (i, j) in zip(mask_row, mask_col):
                output[i,:,j,:] = 0.

            loss = self.loss(output.cuda(), label)
            # due to sum of loss over each elements
            loss = loss / (torch.sum(mask) * data.shape[1] * data.shape[-1])
